%Code used to produce Figurs 10, 11, S8 & S9 - a scatter plot showing the impact of
%randomly varied model parameters on the probability of strain extinction.
function s_basic_many_patches_bypatch_sensitivity_analysis
...
Scatter_Data = zeros(4, Samples); %%%the rows should change depending on the number of parameters varying;
Extinctions_Data = zeros(4,Samples);
...
%     Ci = 0.05;       % cost of inflammation
    ...
    % s = 1e-10;
%     qqC = 0.0004;       % inflammation decay rate
    sC = max(0, normrnd(1e-10, (0.4*1e-10))); %inflammation increase
    qqC = max(0, normrnd(0.0004, (0.4*0.0004))); %inflammation decay
    Cr = max(0, normrnd(0.05, (0.4*0.05))); %cost of resistance
    Ci = max(0, normrnd(0.05, (0.4*0.05))); %cost of inflammation
    ...
    Scatter_Data(:,I) = [sC; qqC; Cr; Ci];
%     Scatter_Data(:,I) = [sC];
    %%%Set up results matrices for sensitivity
    ...
    %
    %Now we loop over several paths
    ...
    for J=1:Paths
        for i=1:tt
            % Start with previous values
                ...
                for ipt=1:n_pat
                    Il(i+1,ipt) = Il(i,ipt) + sC*(x_infl(i+1,ipt)) - qqC*Il(i,ipt);
                end
            ...
            %                     pause
            %                 end
            %                 if (deadbacteria>x_normal(i+1,q))
            %                     deadbacteria
            %                     xn=x_normal(i+1,q)
            %                     pd_actual
            %                     i
        ...
        if (sum(x_infl(tt,pt) == 0))
                    Extinctions_Data(I) = Extinctions_Data(I) + 1;
        end
    end
                if (rem(i,dst)==0)
            %        timestep=i
    ...
    Extinctions_Data = Extinctions_Data/Paths;
    % Set up file names
    fileA='Bacteria_Ks_';
    if (infection==0)
    ...
    fileS='Sensitivity_Parameters_';
    fileEL='Disease_Outcome_';
%     filess='_inflamincrease';
%     fileqq='_inflamdecay';
%     fileCr='_costresistance';
    fileSG='.mat';
    
    fileS1={fileS,fileSG};
    filename2=strjoin(fileS1,'');
    save(filename2, 'Scatter_Data');

    fileS2={fileEL,fileSG};
    filename3=strjoin(fileS2,'');
    save(filename3, 'Extinctions_Data');
...
figure()
scatter(Scatter_Data(1,(1:Samples )), Extinctions_Data(1:Samples ))
plotit = title('s - inflammation increase');
plotit.FontSize = 10;
ylabel({'Probability of clearing infection'; 'by day 30'}, 'FontSize',12)
xlabel('Parameter Value', 'FontSize',12)

figure()
scatter(Scatter_Data(2,(1:Samples )), Extinctions_Data(1:Samples ))
plotit = title('q - inflammation decay');
plotit.FontSize = 10;
ylabel({'Probability of clearing infection'; 'by day 30'}, 'FontSize',12)
xlabel('Parameter Value', 'FontSize',12)

figure()
scatter(Scatter_Data(1,(1:Samples ))./Scatter_Data(2,(1:Samples )), Extinctions_Data(1:Samples ))
plotit = title('s/q');
plotit.FontSize = 10;
ylabel({'Probability of clearing infection'; 'by day 30'}, 'FontSize',12)
xlabel('Parameter Value', 'FontSize',12)
...
figure()
scatter(Scatter_Data(3,(1:Samples )), Extinctions_Data(1:Samples ))
plotit = title('C_r - Cost of resistance');
plotit.FontSize = 10;
ylabel({'Probability of clearing infection'; 'by day 30'}, 'FontSize',12)
xlabel('Parameter Value', 'FontSize',12)

figure()
scatter(Scatter_Data(4,(1:Samples )), Extinctions_Data(1:Samples ))
plotit = title('C_i - cost of inflammation');
plotit.FontSize = 10;
ylabel({'Probability of clearing infection'; 'by day 30'}, 'FontSize',12)
xlabel('Parameter Value', 'FontSize',12)

%%%%%%scatter plots
x=Scatter_Data(2,(1:Samples ))';
y=Scatter_Data(3,(1:Samples ))';
C=Extinctions_Data(1:Samples)';

figure();
scatter3(Scatter_Data(1,(1:Samples ))',Scatter_Data(2,(1:Samples ))',C,30,C, 'filled');
hold on
colormap(cool);
colorbar
xlabel('s - inflammation increase'); ylabel('q - inflammation decay');zlabel({'Probability of clearing infection'; 'by day 30'});
title('Disease outcome')
shading flat;
set(gca, 'Linewidth',2,'Fontsize',16)

figure();
scatter3(Scatter_Data(2,(1:Samples ))',Scatter_Data(3,(1:Samples ))',Extinctions_Data(1:Samples)',30,C,'filled');
% colomap and bar
colormap(cool);
colorbar
xlabel('q - inflammation decay'); ylabel('C_r - cost of resistance');zlabel({'Probability of clearing infection'; 'by day 30'})
set(gca, 'Linewidth',2,'Fontsize',16)
