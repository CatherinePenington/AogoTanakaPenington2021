%%%%%Code used to produce Fig 3,4,5 & Fig S1 

function s_basic_many_patches_bypatch_new_commensal
...
% Cr = 0.005;       % low cost of resistance in Fig 6
% Ci = 0.005;       % low cost of inflammation in Fig 6
...
% qq = 0.0005;      %%%faster decay in Figure S1
% qq = 0.00055;      %%%weak immune response in Figs 4 &5
...
in_normal = 0;  % proportion of new cells that are normal
in_new = 1;     % proportion of new cells that are resistant
...
x_new0 = zeros(1,n_pat);        % new incoming bacteria
    ...
    for i=1:tt
        ...
        for jj=1:8  %%numel(tty)
            ...
            for ii=1:tty(jj)
                ...
                rr1(rr1==0)=1;%%%added since rr1 returns zero values and thus can not be an index 
                rr=notrd(rr1);
                rord(rr)=jj;
